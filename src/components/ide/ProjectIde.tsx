...
export function ProjectIde({
  ...
  const exportZip = async () => {
    ...
    try {
      ...
      for (const p of paths) {
        zip.file(p, files[p]?.content ?? "");
      }
      const blob = await zip.generateAsync({ type: "blob" });
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = `${projectName.replace(/[^\w\-]+/g, "_")}-kod.zip`;
      a.click();
      URL.revokeObjectURL(a.href);
      toast("zip downlowded", "success");
    } catch {
  ...
  const importZip = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const relPath of Object.keys(z.files)) {
        ...
        if (relPath.includes("__MACOSX") || relPath.endsWith(".DS_Store")) {
          continue;
        }
        let path: string;
        try {
          path = sanitizeCodePath(relPath);
        } catch {
          continue;
        }
        ...
        if (!res.ok) {
          const err = await res.json().catch(() => ({}));
          toast(err.error ?? `import stopt at ${path}`, "error");
          await refreshFiles();
          return;
        }
      }
      await refreshFiles();
      toast(`import dun (${newCount} noo filez)`, "success");
    } catch {
  ...
  return (
    <div className="flex h-screen flex-col bg-cream-100 dark:bg-dark-bg">
      <header className="flex shrink-0 items-center gap-3 border-b border-cream-200 px-4 py-2 dark:border-dark-border">
        <button
          type="button"
          onClick={() => router.push(`/projects/${projectId}`)}
          className="flex items-center gap-1 text-sm text-ink-600 hover:text-ink-900 dark:text-cream-400 dark:hover:text-cream-100"
        >
          <ArrowLeft size={16} />
          projek
        </button>
        <span className="text-ink-300 dark:text-cream-600">/</span>
        <h1 className="min-w-0 truncate text-sm font-semibold text-ink-900 dark:text-cream-100">
          {projectName}
        </h1>
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          >
            {importing ? (
              ...
              <FolderArchive size={14} className="mr-1" />
            )}
            import zip
          </Button>
          <input
          ...
          >
            <GitHubIcon size={14} />
            <span className="ml-1">push 2 GitHub</span>
          </Button>
          <Button
            type="button"
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="flex w-56 shrink-0 flex-col border-r border-cream-200 bg-white dark:border-dark-border dark:bg-dark-surface">
          <div className="border-b border-cream-200 px-3 py-2 text-xs font-medium text-ink-500 dark:border-dark-border dark:text-cream-500">
            filez
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto">
            {loading ? (
              ...
              <p className="p-3 text-xs text-ink-400">
                no files yet — add wun wif &quot;noo file&quot;.
              </p>
            ) : (
              <FileTree
                root={tree}
                selectedPath={activePath}
                onSelectFile={(p) => void selectFile(p)}
              />
            )}
          </div>
        </aside>
            ...
            editor={
              <div className="relative h-full min-h-0 min-w-0">
                {activePath ? (
                  ...
                  <div className="flex h-full items-center justify-center text-sm text-ink-500 dark:text-cream-500">
                    {paths.length === 0
                      ? "creat a file 2 start koding"
                      : "peek a file in da tree"}
                  </div>
                )}
              </div>
            }
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={() => void handleCreateFile()} disabled={creating}>
              {creating ? "creatin…" : "creat"}
            </Button>
          </div>
        </div>
      </Modal>
      ...
      <ConfirmDialog
        ...
        title="deleet file"
        message={`deleet “${deletePath ?? ""}”? dis kant b undun.`}
